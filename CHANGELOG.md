...
- Removed the `Substream` associated type from the `ProtocolsHandler` trait. The type of the substream is now always `tet_libp2p::swarm::NegotiatedSubstream`.
...
- Revamped the API of `tet_libp2p_websockets::framed`.
...
- `tet_libp2p::websocket::error::Error` now implements `Sync`.
...
- Removed `BrowserWsConfig`. Please use `tet_libp2p::wasm_ext::ExtTransport` instead.
